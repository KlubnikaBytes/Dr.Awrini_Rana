...
const Clinic = require('./models/Clinic'); // Assuming Clinic model exists
...
async function seed() {
  try {
    ...
    console.log('MongoDB connected for seeding LabCatalog');

    // Get clinics
    const clinics = await Clinic.find();
    if (clinics.length === 0) {
      console.log('No clinics found. Cannot seed without a clinic ID.');
      process.exit(1);
    }

    for (let clinic of clinics) {
      console.log(`Seeding lab catalog for clinic: ${clinic._id}`);
      for (const [category, tests] of Object.entries(TEST_CATEGORIES)) {
        const existing = await LabCatalog.findOne({ clinicId: clinic._id, category });
        if (!existing) {
          await LabCatalog.create({
            clinicId: clinic._id,
            category,
            tests
          });
